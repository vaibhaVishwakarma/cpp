...
int main(void){
    ...
    for(int i = 0 ; i<V ; i++){
        for(int v = 0 ; v<V ; v++){
            if(adj[i][v] == INT_MAX || i == v) continue;
            push(que,{v,adj[i][v],i});
        }
    }
    printf("Edges in the constructed MST:\n");
    while(getsum(visited)<V){
        sortq(que);
        tri it = pop(que);
        int curr = it.first , parent = it.third , weight = it.second;
        if(curr == parent) continue;
        if(visited[curr] && visited[parent]) continue;
        visited[curr] = 1;
        visited[parent]= 1;
        wt += weight;
        printf("%d -- %d == %d\n",parent , curr , weight);
    }
    printf("Minimum Spanning Tree cost: %d",wt);
